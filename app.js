...
app.post('/api/meeting-active', authenticateUser, async (req, res) =>{//req {meetingId,  }
  try{
    ...
    scheduleSnapshot.forEach(doc =>{
      doc.data().meetings.forEach(mtd => {
        if(mtd.meetingId === req.body.meetingId){
          ...
          const currentDate = moment(vietnamTime);
    ...
    const durationInSecond = currentTime + timeExpire.diff(moment(vietnamTime), "second");
    ...
    return res.status(200).json({
      ...
      token: token,
      channelName,
      timeOutSeconds: timeExpire.diff(moment(vietnamTime), "second")
    })
  }catch(error){
    res.status(500).json({message: error.message});
  }
